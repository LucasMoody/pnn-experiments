...
def get_results_for_task(df, task, source_tasks, aggfuncs, minSize, maxSize, pnn_arch='pnn', suffix=''):
    ...
    tmp = df[(df.score_dev != 0) & (df.task == task) & (df.transfer.isin(source_tasks + ['none'] + multitask_transfer_spec)) & (df['sample'] >= minSize) & (df['sample'] <= 10000000) & (df['exp'].isin(['baseline','finetuning','pipeline','multitask', pnn_arch]))]
